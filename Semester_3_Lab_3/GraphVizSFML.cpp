...
#ifdef USE_SFML
// Тихо ищем любой доступный шрифт; без спама в консоль.
...
void VisualizeGraphSFML(const AdjListGraph& g) {
    ...
    sf::Font font;
    const bool hasFont = LoadAnyFont(font); // тихо; если не нашли — подписи отключатся
    ...
    while (window.isOpen()) {
        ...
        for (auto [u, v] : edges) {
            ...
            seg[0].color = seg[1].color = sf::Color(180,180,180);
            window.draw(seg, 2, sf::PrimitiveType::Lines);
        }

        // вершины + НОМЕРА 0..n-1 (если шрифт найден)
        ...
        for (int v = 0; v < n; ++v) {
            ...
            if (hasFont) {
                // простое «почти по центру» позиционирование
